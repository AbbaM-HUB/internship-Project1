...
// .wrangler/tmp/bundle-HjA7hW/checked-fetch.js
...
app.get("/ws", async (c) => {
  ...
  if (!profile) {
    console.log(`\u{1F680} KV Cache MISS for: ${userEmail}`);
    profile = {
      email: userEmail,
    ...
    console.log(`\u{1F4E6} KV Cache HIT for: ${userEmail}`);
  }
...
var BoxRoom = class {
  ...
  async fetch(request) {
    ...
    server.send(JSON.stringify({ type: "init", ...pos }));
